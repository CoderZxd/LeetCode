...
//        List<List<Integer>>  result = pairs.palindromePairs(new String[]{"abcd","dcba","lls","s","sssll"});
    ...
    private void check(String[] words,int i,int j,List<List<Integer>> resultList){
        String w1 = words[i];
        String w2 = words[j];
        String jionWord = w1+w2;
        int lenWord = jionWord.length();
        if(jionWord.charAt(0) == jionWord.charAt(lenWord - 1)){
            if(lenWord % 2 == 0){
                StringBuilder sb1 = new StringBuilder(jionWord.substring(0,lenWord/2));
                StringBuilder sb2 = new StringBuilder(jionWord.substring(lenWord/2));
                ...
                StringBuilder sb1 = new StringBuilder(jionWord.substring(0,lenWord/2));
                StringBuilder sb2 = new StringBuilder(jionWord.substring(lenWord/2+1));
                if(sb1.reverse().toString().equals(sb2.toString())){
                    List<Integer> tempList = new ArrayList<>(2);
                    tempList.add(i);
                    tempList.add(j);
